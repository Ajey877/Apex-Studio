  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (engine) {
        ...
        setInputLevel(peak);
        if (engine.getState() === 'recording' || engine.getState() === 'paused') {
          setRecordSeconds(engine.getDurationSeconds());
        }
      ...
      <div className="bg-[#141416] border border-[#333336] rounded-xl w-full max-w-lg shadow-2xl overflow-hidden text-[#b0b0b0]">
        <div className="px-5 py-3.5 bg-[#1a1a1d] border-b border-[#333336] flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm text-white tracking-tight">AUDIO RECORDER</h3>
              <p className="text-[10px] text-[#777]">Real microphone / line-in capture</p>
            </div>
          </div>
          <button onClick={onClose} disabled={recordingState === 'stopping'} className="p-1 rounded hover:bg-[#2d2d30] text-[#777] hover:text-white transition disabled:opacity-40">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          <div className="bg-[#0a0a0b] border border-[#333336] rounded-lg p-4 flex flex-col items-center justify-center space-y-2">
            <div className="text-2xl font-mono font-bold text-white flex items-center gap-2">
              {(recordingState === 'recording' || recordingState === 'paused') && <div className="w-3 h-3 bg-[#ff0000] rounded-full animate-pulse" />}
            ...
            <div className="w-full flex items-center justify-between text-[10px] text-[#777] font-mono">
              <span>INPUT PEAK: {Math.round(inputLevel * 100)}%</span>
              <span className={(recordingState === 'recording' || recordingState === 'paused') ? 'text-[#ff6e00] font-bold' : ''}>
          ...
          {error && (
            <div className="flex items-start gap-2 p-3 bg-red-950/30 border border-red-800/50 rounded-lg text-xs text-red-200">
          ...
          <div className="flex items-center justify-center gap-3">
            {recordingState === 'idle' && (
              <button onClick={handleStart} className="flex items-center gap-2 px-6 py-2.5 bg-[#ff6e00] hover:bg-[#ff7d1a] text-black font-bold text-sm rounded shadow-lg transition active:scale-95">
            ...
            {(recordingState === 'recording' || recordingState === 'paused') && (
              <>
                <button onClick={handlePauseResume} className="flex items-center gap-2 px-5 py-2.5 bg-[#222225] hover:bg-[#2d2d30] text-white font-bold text-sm rounded border border-[#333336] transition">
                  {recordingState === 'recording' ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                  {recordingState === 'recording' ? 'PAUSE' : 'RESUME'}
                </button>
                <button onClick={handleStop} className="flex items-center gap-2 px-6 py-2.5 bg-[#ff0000] hover:bg-red-600 text-white font-bold text-sm rounded shadow-lg transition active:scale-95">
          ...
          {recordedTake && (
            <div className="p-3 bg-[#1a1a1d] border border-[#ff6e00]/50 rounded-lg space-y-3">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-[#ff6e00] font-mono font-bold">{recordedTake.durationSeconds.toFixed(2)}s</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-[#777]">Insert onto Track Lane:</span>
                <select value={targetTrack} onChange={e => setTargetTrack(Number(e.target.value))} className="bg-[#121214] text-white text-xs px-2.5 py-1 rounded border border-[#333336] focus:outline-none">
              ...
              <div className="h-8 flex items-end gap-px bg-[#0a0a0b] rounded px-1 overflow-hidden">
                {recordedTake.waveform.slice(0, 128).map((value, index) => (
                  <div key={index} className="flex-1 bg-[#ff6e00]" style={{ height: `${Math.max(2, value * 100)}%` }} />
                ))}
              </div>
              <button onClick={handleApplyToPlaylist} className="w-full py-2 bg-[#ff6e00] hover:bg-[#ff7d1a] text-black font-bold text-xs rounded transition flex items-center justify-center gap-1.5">
                <Check className="w-4 h-4" /> PLACE TAKE ON PLAYLIST
              </button>
            </div>
          )}
        </div>
