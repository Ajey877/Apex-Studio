...
describe('TrackRegistry', () => {
  it('creates deterministic unique track IDs', () => {
  ...
  it('does not reuse an ID after removal', () => {
    const registry = new TrackRegistry();
  ...
  it('rejects duplicate IDs', () => {
    ...
    assert.throws(() => registry.add({ id: 0, name: 'Duplicate', volumeDb: 0, pan: 0, muted: false, soloed: false }), /already exists/);
  });

  it('updates state without exposing internal references', () => {
    const registry = new TrackRegistry();
    ...
    assert.equal(registry.get(created.id)?.name, 'Lead Vocal');
    assert.notEqual(registry.get(created.id), renamed);
  });

  it('clamps supported mixer values through the existing channel contract', () => {
    const registry = new TrackRegistry();
  ...
  it('rejects empty names and missing tracks', () => {
    const registry = new TrackRegistry();
