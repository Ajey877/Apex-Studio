...
test('subtractive synth renders independently and returns a stoppable voice', () => {
  ...
  assert.ok(handle);
  assert.ok(context.operations.some(([name]) => name === 'createOscillator'));
  assert.ok(context.operations.some(([name]) => name === 'createBiquadFilter'));
