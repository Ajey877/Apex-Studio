 ...
 * Track IDs are unique and never reused automatically.
 */
export class TrackRegistry {
  ...
  add(track: MixerChannelState): void {
    if (this.tracksById.has(track.id)) {
      throw new Error(`Track ID ${track.id} already exists.`);
    }
  ...
  snapshot(): TrackRegistrySnapshot {
    return { tracks: this.list() };
  }

  remove(id: number): boolean {
    return this.tracksById.delete(id);
  }
  ...
  private update(
    id: number,
    updater: (track: MixerChannelState) => MixerChannelState,
  ): MixerChannelState {
  ...
  private validateTrack(track: MixerChannelState): void {
    if (!Number.isInteger(track.id) || track.id < 0) {
      throw new Error('Track ID must be a non-negative integer.');
    }
    if (!track.name.trim()) throw new Error('Track name cannot be empty.');
    if (!Number.isFinite(track.volumeDb) || track.volumeDb < -60 || track.volumeDb > 12) {
      throw new Error('Track volume must be between -60 dB and +12 dB.');
    }
    if (!Number.isFinite(track.pan) || track.pan < -1 || track.pan > 1) {
