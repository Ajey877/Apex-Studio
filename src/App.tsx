...
export function App() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space') { e.preventDefault(); handleTogglePlay(); return; }
      if (e.key === 'l' || e.key === 'L') { handleTogglePlayMode(); return; }
      if (e.key === 'r' || e.key === 'R') { handleToggleRecord(); return; }
      if (e.key === 'm' || e.key === 'M') { setMetronome(m => !m); return; }
      if (e.code === 'Numpad0' || e.key === '0' || e.code === 'Home') { handleStop(); return; }
      if (e.key === '1' || e.code === 'F6') { e.preventDefault(); setCurrentView('channel_rack'); return; }
      if (e.key === '2' || e.code === 'F7') { e.preventDefault(); setCurrentView('piano_roll'); return; }
      if (e.key === '3' || e.code === 'F5') { e.preventDefault(); setCurrentView('playlist'); return; }
      if (e.key === '4' || e.code === 'F9') { e.preventDefault(); setCurrentView('mixer'); return; }
      if (e.key === '5' || e.code === 'F8') { e.preventDefault(); setCurrentView('instruments'); return; }
      if (e.code === 'KeyZ') { setKeyboardOctave(prev => Math.max(-2, prev - 1)); return; }
      if (e.code === 'KeyX') { setKeyboardOctave(prev => Math.min(2, prev + 1)); return; }
      if (KEY_NOTE_MAP[e.code] !== undefined && !activeHeldKeysRef.current.has(e.code) && !e.repeat) {
        ...
        if (currentChan) {
          audioEngine.playNote(currentChan, { id: `key-${e.code}-${Date.now()}`, pitch, start: 0, duration: 1.2, velocity: 0.95 });
        }
      }
    };

    const handleKeyUp = (e: KeyboardEvent) => {
      if (activeHeldKeysRef.current.has(e.code)) activeHeldKeysRef.current.delete(e.code);
    };
  ...
  const handleAuditionSample = (name: string, pitch = 60) => {
    ...
    if (selectedChannel) {
      audioEngine.playNote(selectedChannel, { id: `prev-${Date.now()}`, pitch, start: 0, duration: 0.8, velocity: 0.9 });
    }
  ...
  return (
    <div id="phantom-mobile-daw" className="bg-[#0a0a0b] text-[#b0b0b0] h-screen w-screen flex flex-col font-sans select-none overflow-hidden">
      <TransportBar currentView={currentView} onSelectView={(v) => setCurrentView(v)} isPlaying={isPlaying} onTogglePlay={handleTogglePlay} onStop={handleStop} playMode={playMode} onTogglePlayMode={handleTogglePlayMode} isRecording={isRecording} onToggleRecord={handleToggleRecord} meta={projectState.meta} onUpdateMeta={handleUpdateMeta} currentStep={currentStep} currentBar={currentBar} metronome={metronome} onToggleMetronome={() => setMetronome(!metronome)} onOpenExport={() => setIsExportOpen(true)} onOpenProjectManager={() => setIsProjectManagerOpen(true)} onOpenCollab={() => setIsCollabOpen(true)} onOpenAnalytics={() => setIsAnalyticsOpen(true)} onOpenSubscription={() => setIsSubscriptionOpen(true)} onOpenHotkeys={() => setIsHotkeysOpen(true)} onOpenMidi={() => setIsMidiModalOpen(true)} onOpenParametricEq={() => { setEqModalTrackId(0); setIsParametricEqOpen(true); }} onOpenGrossBeat={() => setIsGrossBeatOpen(true)} onOpenSlicer={() => setIsAudioSlicerOpen(true)} onOpenMasteringSuite={() => setIsMasteringSuiteOpen(true)} onOpenSampleManager={() => setIsSampleManagerOpen(true)} onOpenVocalTuner={() => setIsVocalTunerOpen(true)} onOpenMidiLearn={() => setIsMidiLearnOpen(true)} onOpenMultiZoneSampler={() => setIsMultiZoneSamplerOpen(true)} onOpenWavetableSynth={() => setIsWavetableSynthOpen(true)} onOpenWamPlugin={() => setIsWamPluginOpen(true)} onOpenTakeComping={() => setIsTakeCompingOpen(true)} onOpenSidechain={() => setIsSidechainOpen(true)} onOpenPolyphonicEditor={() => setIsPolyphonicEditorOpen(true)} onOpenDesktopApp={() => setIsDesktopAppOpen(true)} onOpenWarpProcessor={() => setIsWarpProcessorOpen(true)} onOpenVideoScoring={() => setIsVideoScoringOpen(true)} onOpenSpatialAudio={() => setIsSpatialAudioOpen(true)} onOpenMpeExpression={() => setIsMpeExpressionOpen(true)} onOpenStemSplitter={() => setIsStemSplitterOpen(true)} onOpenMasterMacros={() => setIsMasterMacrosOpen(true)} onOpenProjectZipBundle={() => setIsProjectZipOpen(true)} collaboratorCount={collaborators.length} isProUser={isProUser} isSidebarOpen={isSidebarOpen} onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
      <main className="flex-1 flex overflow-hidden">
        {isSidebarOpen && <aside className="w-56 md:w-64 bg-[#141416] border-r border-[#333336] flex flex-col shrink-0"><div className="p-2.5 border-b border-[#333336] flex justify-between items-center bg-[#1a1a1d]"><span className="text-[10px] font-bold uppercase tracking-widest text-[#777]">STUDIO BROWSER</span><button onClick={() => setIsProjectManagerOpen(true)} className="text-[#ff6e00] hover:text-white text-[10px] font-bold transition">+ New</button></div><div className="p-2 border-b border-[#333336] bg-[#121214]"><div className="flex items-center gap-1.5 bg-[#1a1a1d] px-2 py-1 rounded border border-[#333336]"><Search className="w-3 h-3 text-[#777]" /><input type="text" placeholder="Search samples & VSTs..." value={browserSearch} onChange={(e) => setBrowserSearch(e.target.value)} className="w-full bg-transparent text-[11px] text-white placeholder-[#555] focus:outline-none" /></div></div><div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-3 text-xs"><div className="space-y-1"><div onClick={() => setExpandedFolders(f => ({ ...f, instruments: !f.instruments }))} className="flex items-center justify-between text-[10px] font-bold text-[#777] hover:text-white cursor-pointer px-1"><span className="flex items-center gap-1"><Cpu className="w-3 h-3 text-[#ff6e00]" /><span>SYNTHS & GENERATORS</span></span>{expandedFolders.instruments ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}</div>{expandedFolders.instruments && <div className="space-y-0.5 pl-3 border-l border-[#222225] mt-1">{[{ name: 'Grand Concert Piano', type: 'grand_piano', color: '#e0e0e0' },{ name: 'Vintage Rhodes MK1', type: 'rhodes_epiano', color: '#e67e22' },{ name: 'Hammond B3 Organ', type: 'hammond_organ', color: '#d35400' },{ name: 'Orchestral Strings', type: 'strings_ensemble', color: '#9b59b6' },{ name: 'Pizzicato Strings', type: 'pizzicato_strings', color: '#8e44ad' },{ name: 'Nylon Pluck Guitar', type: 'nylon_guitar', color: '#27ae60' },{ name: 'Cinematic Horns/Brass', type: 'cinematic_brass', color: '#f39c12' },{ name: '808 Tuned Sub Bass', type: 'sub_808', color: '#ff5722' },{ name: 'TB-303 Acid Bassline', type: 'acid_303', color: '#2ecc71' },{ name: 'Reese Heavy Bass', type: 'reese_bass', color: '#c0392b' },{ name: 'JP-8000 Supersaw', type: 'supersaw_lead', color: '#00d2d3' },{ name: 'Atmospheric Pad', type: 'ambient_pad', color: '#54a0ff' },{ name: 'Vocal Choir Formant', type: 'vox_choir', color: '#ff9ff3' },{ name: 'Wooden Marimba/Bell', type: 'marimba_bell', color: '#1dd1a1' },{ name: '8-Bit Retro Chiptune', type: 'chiptune_8bit', color: '#feca57' },{ name: 'MiniSynth Subtractive', type: 'minisynth', color: '#ff6e00' },{ name: 'Toxic FM Synthesizer', type: 'fmsynth', color: '#00bcd4' },{ name: 'DirectWave Sampler', type: 'sampler', color: '#4caf50' },{ name: '808 Drum Machine', type: 'drumpad', color: '#ff5722' }].map((item, i) => <div key={i} onClick={() => handleAddChannel(item.type as InstrumentType, item.name, item.color)} className="flex items-center justify-between px-2 py-1 rounded hover:bg-[#222225] text-[11px] text-zinc-300 hover:text-white cursor-pointer group"><span className="truncate">{item.name}</span><span className="text-[9px] text-[#ff6e00] opacity-0 group-hover:opacity-100 font-bold">+ LOAD</span></div>)}</div>}</div><div className="space-y-1"><div onClick={() => setExpandedFolders(f => ({ ...f, drums: !f.drums }))} className="flex items-center justify-between text-[10px] font-bold text-[#777] hover:text-white cursor-pointer px-1"><span className="flex items-center gap-1"><Disc className="w-3 h-3 text-[#ff6e00]" /><span>DRUM SAMPLES (808 / MPC)</span></span>{expandedFolders.drums ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}</div>{expandedFolders.drums && <div className="space-y-0.5 pl-3 border-l border-[#222225] mt-1">{[{ name: '808_Sub_Punch.wav', pitch: 36 },{ name: 'Snare_Trap_Hard.wav', pitch: 38 },{ name: 'HiHat_Closed_Tight.wav', pitch: 42 },{ name: 'Clap_Studio_Dry.wav', pitch: 39 },{ name: 'Perc_Rimshot_Wood.wav', pitch: 37 }].map((sample, i) => <div key={i} onClick={() => handleAuditionSample(sample.name, sample.pitch)} className={`flex items-center justify-between px-2 py-1 rounded text-[11px] cursor-pointer transition ${previewingAudio === sample.name ? 'bg-[#ff6e00] text-black font-bold' : 'hover:bg-[#222225] text-zinc-300 hover:text-white'}`}><span className="truncate">{sample.name}</span><Play className="w-2.5 h-2.5 opacity-60" /></div>)}</div>}</div><div className="space-y-1"><div onClick={() => setExpandedFolders(f => ({ ...f, presets: !f.presets }))} className="flex items-center justify-between text-[10px] font-bold text-[#777] hover:text-white cursor-pointer px-1"><span className="flex items-center gap-1"><Folder className="w-3 h-3 text-[#ff6e00]" /><span>STUDIO DEMOS</span></span>{expandedFolders.presets ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}</div>{expandedFolders.presets && <div className="space-y-0.5 pl-3 border-l border-[#222225] mt-1">{PRESET_PROJECTS.map((p, i) => <div key={i} onClick={() => setProjectState(p.state)} className="flex items-center justify-between px-2 py-1 rounded hover:bg-[#222225] text-[11px] text-zinc-300 hover:text-white cursor-pointer group"><span className="truncate">{p.name}</span><span className="text-[9px] text-[#ff6e00] font-mono">{p.bpm} BPM</span></div>)}</div>}</div></div><div className="p-3 border-t border-[#333336] bg-[#1a1a1d] space-y-1.5"><div className="flex justify-between items-center text-[9px] font-bold"><span className="text-white">ACTIVE PREVIEW:</span><span className="text-[#ff6e00] font-mono">{previewingAudio ? 'AUDITIONING' : 'READY'}</span></div><div className="h-6 bg-[#0a0a0b] rounded border border-[#333336] flex items-center px-1.5 gap-0.5 overflow-hidden">{Array.from({ length: 28 }).map((_, i) => <div key={i} className={`flex-1 rounded-xs transition-all ${previewingAudio ? 'bg-[#ff6e00]' : 'bg-[#333336]'}`} style={{ height: `${20 + (i % 7) * 12}%` }} />)}</div></div></aside>}
        <section className="flex-1 flex flex-col bg-[#121214] overflow-hidden">
          {currentView === 'channel_rack' && <ChannelRack channels={projectState.channels} patterns={projectState.patterns} selectedPatternId={projectState.selectedPatternId} onSelectPattern={(id) => setProjectState(prev => ({ ...prev, selectedPatternId: id }))} onAddPattern={handleAddPattern} selectedChannelId={selectedChannelId} onSelectChannel={(id) => setSelectedChannelId(id)} onUpdateChannel={handleUpdateChannel} onAddChannel={handleAddChannel} onDeleteChannel={handleDeleteChannel} onOpenPianoRoll={(id) => { setSelectedChannelId(id); setCurrentView('piano_roll'); }} onOpenInstrument={(id) => { setSelectedChannelId(id); setCurrentView('instruments'); }} onOpenArp={(id) => { setArpChannelId(id); setIsArpeggiatorOpen(true); }} onOpenSampleManager={(id) => { setSampleChannelId(id); setIsSampleManagerOpen(true); }} currentStep={currentStep} isPlaying={isPlaying} swing={projectState.meta.swing} onUpdateSwing={(swing) => handleUpdateMeta({ swing })} />}
          {currentView === 'piano_roll' && selectedChannel && <PianoRoll channel={selectedChannel} allChannels={projectState.channels} onSelectChannel={(id) => setSelectedChannelId(id)} onUpdateChannel={handleUpdateChannel} currentStep={currentStep} isPlaying={isPlaying} />}
          {currentView === 'playlist' && <PlaylistArranger tracks={projectState.playlistTracks} clips={projectState.playlistClips} patterns={projectState.patterns} channels={projectState.channels} markers={projectState.markers || []} onUpdateTracks={handleUpdateTracks} onUpdateClips={handleUpdateClips} onAddTrack={handleAddPlaylistTrack} onUpdateMarkers={(markers) => setProjectState(prev => ({ ...prev, markers }))} onSeekToBar={(bar) => setCurrentBar(bar)} currentBar={currentBar} isPlaying={isPlaying} />}
          {currentView === 'mixer' && <Mixer tracks={projectState.mixerTracks} selectedTrackId={selectedTrackId} onSelectTrack={(id) => setSelectedTrackId(id)} onUpdateTrack={handleUpdateMixerTrack} onAddFxSlot={handleAddFxSlot} onDeleteFxSlot={handleDeleteFxSlot} onUpdateFxSlot={handleUpdateFxSlot} isPlaying={isPlaying} onOpenParametricEq={(track) => { setEqModalTrackId(track.id); setIsParametricEqOpen(true); }} />}
          {currentView === 'instruments' && selectedChannel && <InstrumentRack channel={selectedChannel} allChannels={projectState.channels} onSelectChannel={(id) => setSelectedChannelId(id)} onUpdateChannel={handleUpdateChannel} />}
          {currentView === 'sampler' && <div className="flex flex-col h-full items-center justify-center p-8 bg-[#121214] text-center space-y-4"><div className="w-16 h-16 bg-[#ff6e00]/15 border border-[#ff6e00]/30 rounded-2xl flex items-center justify-center text-[#ff6e00]"><Volume2 className="w-8 h-8" /></div><div><h2 className="text-xl font-bold text-white tracking-tight">DIRECTWAVE AUDIO SAMPLER & VOCAL CAPTURE</h2><p className="text-xs text-[#777] max-w-md mt-1">High-fidelity 48kHz Direct-to-Disk recording station with automatic waveform slicing and transient detection.</p></div><div className="flex items-center gap-3 pt-2"><button onClick={() => setIsAudioRecorderOpen(true)} className="px-5 py-2.5 bg-[#ff6e00] hover:bg-[#ff7d1a] text-black font-bold text-xs rounded transition flex items-center gap-2 shadow"><div className="w-2.5 h-2.5 bg-black rounded-full" /><span>OPEN MICROPHONE RECORDER</span></button><button onClick={() => setIsSampleManagerOpen(true)} className="px-5 py-2.5 bg-[#00ff88] hover:bg-[#00e67a] text-black font-bold text-xs rounded transition flex items-center gap-2 shadow"><Volume2 className="w-3.5 h-3.5" /><span>DIRECTWAVE SAMPLE LOADER</span></button><button onClick={() => setCurrentView('instruments')} className="px-5 py-2.5 bg-[#222225] hover:bg-[#2d2d30] text-white font-bold text-xs rounded border border-[#333336] transition">Open VST Synthesizers</button></div></div>}
        </section>
      </main>
      <footer className="h-6 bg-[#1a1a1d] border-t border-[#333336] flex items-center px-4 justify-between shrink-0 select-none"><div className="flex items-center gap-4 text-[9px]"><span className="text-[#777]">SYNC: <span className="text-[#00ff00] font-bold">ONLINE (E2EE)</span></span><span className="text-[#777]">DSP CPU: <span className="text-white font-bold">{isPlaying ? '18%' : '8%'}</span></span><span className="text-[#777]">LATENCY: <span className="text-white font-bold">2.4ms (LOW)</span></span><span className="text-[#777] hidden md:inline">PROJECT: <span className="text-[#ff6e00] font-bold">{projectState.meta.name}</span></span></div><div className="flex items-center gap-3 text-[9px] font-bold text-[#777]"><button onClick={() => setIsHotkeysOpen(true)} className="hover:text-white transition cursor-pointer">HOTKEYS (SPACE / 1-5)</button><span className="w-1 h-1 bg-[#444] rounded-full"></span><button onClick={() => setIsExportOpen(true)} className="hover:text-white transition cursor-pointer">EXPORT MASTER</button><span className="w-1 h-1 bg-[#444] rounded-full"></span><button onClick={() => setIsSubscriptionOpen(true)} className="text-[#ff6e00] hover:text-[#ff7d1a] transition cursor-pointer">{isProUser ? 'PRO SUITE ACTIVE' : 'PREMIUM TIER'}</button></div></footer>
      ...
      {(() => { const targetArpChannel = projectState.channels.find(c => c.id === arpChannelId) || projectState.channels[0]; return targetArpChannel ? <ArpeggiatorModal isOpen={isArpeggiatorOpen} onClose={() => setIsArpeggiatorOpen(false)} channel={targetArpChannel} onUpdateChannel={(updatedCh) => handleUpdateChannel(updatedCh.id, updatedCh)} bpm={projectState.meta.bpm} /> : null; })()}
      ...
      <VocalTunerModal isOpen={isVocalTunerOpen} onClose={() => setIsVocalTunerOpen(false)} vocalTunerSettings={projectState.vocalTunerSettings || { enabled: true, rootKey: 0, scale: 'minor', retuneSpeedMs: 15, formantShift: 0, humanize: 0.3, pitchCorrectionAmount: 0.85, vibratoDepth: 0.2 }} onUpdateVocalTuner={(settings) => setProjectState(prev => ({ ...prev, vocalTunerSettings: settings }))} channels={projectState.channels} />
